...
<template>
	<div class="answer">
		...
		<template v-else-if="questionType === 'video' && answers.length">
			<div v-for="answer of answers" :key="answer.id" class="answer__video">
				<video :src="answer.url" controls playsinline preload="metadata" />
				<a :href="`${answer.url}?download=true`">
					<NcIconSvgWrapper :svg="IconDownload" inline />
					{{
						t('forms', 'Download {fileName}', { fileName: answer.text })
					}}
				</a>
			</div>
		</template>
