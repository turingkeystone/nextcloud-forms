...
class PageController extends Controller {
	...
	public function index(?string $hash = null, ?int $submissionId = null): TemplateResponse {
		...
		return $this->setVideoCSP(new TemplateResponse($this->appName, self::TEMPLATE_MAIN, [
	...
	protected function provideTemplate(string $template, ?Form $form = null, array $options = []): TemplateResponse {
		...
		if (!$this->userSession->isLoggedIn()) {
			...
			return $this->setVideoCSP($response);
		}

		return $this->setVideoCSP(new TemplateResponse($this->appName, $template, array_merge([
			'id-app-content' => '#app-content-vue',
			'id-app-navigation' => '#app-navigation-vue',
		], $options)));
	}
	...
	/**
	 * Allow page-local recording Blob URLs in video elements without widening
	 * any other Content Security Policy directive.
	 */
	protected function setVideoCSP(TemplateResponse $response): TemplateResponse {
