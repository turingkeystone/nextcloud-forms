 ...
 * @psalm-method 'date'|'datetime'|'dropdown'|'file'|'long'|'multiple'|'multiple_unique'|'short'|'time'|'video' getType()
 * @method void setType(string $value)
 * @psalm-method void setType('date'|'datetime'|'dropdown'|'file'|'long'|'multiple'|'multiple_unique'|'short'|'time'|'video' $value)
...
class Question extends Entity {
	...
	public function setExtraSettings(array $extraSettings): void {
		...
		foreach ($extraSettings as $key => $value) {
			// Video selection defaults to enabled, so its explicit false value is meaningful.
			if ($value === false && $key !== 'allowExistingVideo') {
