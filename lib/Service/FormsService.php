...
class FormsService {
	...
	public function getQuestions(int $formId): array {
		...
		try {
			...
			foreach ($questionEntities as $questionEntity) {
				...
				} elseif ($question['type'] === Constants::ANSWER_TYPE_VIDEO) {
					$question['accept'] = ['video/*'];
				}
	...
	public function areExtraSettingsValid(array $extraSettings, string $questionType): bool {
		...
		$allowed = match ($questionType) {
			...
			Constants::ANSWER_TYPE_VIDEO => Constants::EXTRA_SETTINGS_VIDEO,
		...
		} elseif ($questionType === Constants::ANSWER_TYPE_VIDEO) {
			$allowedResolutions = ['720p', '1080p', '1440p', '2160p'];
			$allowedOrientations = ['landscape', 'portrait', 'any'];
			if (isset($extraSettings['allowedResolutions'])
				&& (!array_is_list($extraSettings['allowedResolutions'])
					|| count($extraSettings['allowedResolutions']) === 0
					|| count(array_filter($extraSettings['allowedResolutions'], 'is_string')) !== count($extraSettings['allowedResolutions'])
					|| count(array_unique($extraSettings['allowedResolutions'])) !== count($extraSettings['allowedResolutions'])
					|| array_diff($extraSettings['allowedResolutions'], $allowedResolutions))) {
				return false;
			}
			if (isset($extraSettings['defaultResolution'])
				&& (!in_array($extraSettings['defaultResolution'], $allowedResolutions, true)
					|| isset($extraSettings['allowedResolutions'])
						&& !in_array($extraSettings['defaultResolution'], $extraSettings['allowedResolutions'], true))) {
				return false;
			}
			if (isset($extraSettings['maxRecordingDurationSeconds'])
				&& ($extraSettings['maxRecordingDurationSeconds'] < 60
					|| $extraSettings['maxRecordingDurationSeconds'] > Constants::VIDEO_MAX_RECORDING_DURATION_SECONDS)) {
				return false;
			}
			if (isset($extraSettings['recordingOrientation'])
				&& !in_array($extraSettings['recordingOrientation'], $allowedOrientations, true)) {
				return false;
			}
		}
		return true;
