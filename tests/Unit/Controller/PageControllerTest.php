	...
	public function testSetEmbeddedCSP() {
		...
		$response->expects($this->once())
			...
			->with(self::callback(fn (ContentSecurityPolicy $csp): bool => preg_match('/frame-ancestors[^;]* \*[ ;]/', $csp->buildPolicy()) !== false
				&& preg_match('/media-src[^;]* blob:[ ;]/', $csp->buildPolicy()) !== false
				&& preg_match('/(?:script|connect|object)-src[^;]* blob:/', $csp->buildPolicy()) === 0));
		TestCase::invokePrivate($this->pageController, 'setEmbeddedCSP', [$response]);
	}

	public function testSetVideoCSP(): void {
		/** @var MockObject */
		$response = $this->createMock(TemplateResponse::class);
		$response->expects($this->once())
			->method('setContentSecurityPolicy')
			->with(self::callback(fn (ContentSecurityPolicy $csp): bool => preg_match('/media-src[^;]* blob:[ ;]/', $csp->buildPolicy()) !== false
				&& preg_match('/(?:script|connect|object)-src[^;]* blob:/', $csp->buildPolicy()) === 0));

		TestCase::invokePrivate($this->pageController, 'setVideoCSP', [$response]);
	}
}
