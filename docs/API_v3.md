...
"data": [
  {
    ...
    "uploadToken": "<upload-token-from-response>"
  }
]
```
...
Video questions use a capability-token upload session so authenticated and public-link respondents can transfer large browser-backed files without one oversized request. The maximum video size is 16 GiB and the server-selected chunk size is currently 8 MiB.

1. Create a session with `POST /api/v3/forms/{formId}/submissions/videos/{questionId}/upload-sessions` and a JSON body containing `fileName`, `mimeType`, `totalSize`, and optional `shareHash`.
2. Upload every chunk with `POST /api/v3/video-upload-sessions/{sessionId}/chunks/{index}` as `multipart/form-data`. Include the binary `chunk`, the session's `uploadToken`, and an exact `contentRange` value such as `bytes 0-8388607/12582912`. The legacy raw-body `PUT` endpoint with `X-Forms-Upload-Token` and `Content-Range` headers remains available for compatibility.
3. Optionally inspect progress with `GET /api/v3/video-upload-sessions/{sessionId}` and the same token header.
4. Finalize with `POST /api/v3/video-upload-sessions/{sessionId}/complete` and include `uploadToken` in the JSON body. The legacy token header is also accepted. The response has the existing uploaded-file answer shape.
5. Cancel and remove chunks with `DELETE /api/v3/video-upload-sessions/{sessionId}`.

Create response:

```json
{
	"sessionId": "high-entropy-session-id",
	"uploadToken": "one-session-capability-token",
	"chunkSize": 8388608,
	"expires": 1787594400
}
```
...
```json
{
	...
	"uploadToken": "submission-answer-token"
}
```

Chunk retries are idempotent when their expected size matches. Sessions expire after two hours. The session token and both upload tokens are secrets and must not be logged or reused for another form or question.
...
```
  {
    ...
    "3":[                     // File-Upload
      {"uploadedFileId": 42, "uploadToken": "<upload-token-from-response>"},
      {"uploadedFileId": 43, "uploadToken": "<second-upload-token-from-response>"}
  ],
}
```
